...
import { 
  Phone, Search, Filter, Calendar,
  ArrowUpCircle, ArrowDownCircle, Download
} from 'lucide-react';

const PhonePe = ({ transactions, isDark }) => {
  ...
  const filteredTransactions = useMemo(() => {
    let filtered = phonePeTransactions;
    ...
    return filtered;
  }, [phonePeTransactions, searchTerm, filterType]);
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
          ...
          <div className="flex items-center space-x-4">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search PhonePe..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className={`w-40 pl-10 pr-4 py-1.5 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm ${
                  isDark 
            ...
            <select
              ...
              className={`px-3 py-1.5 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent ${
                isDark 
                  ? 'bg-gray-700 text-white border-gray-600' 
                  : 'bg-gray-50 text-gray-800 border-gray-300'
              }`}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className={`p-3 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
            ...
            <div className="text-lg font-bold text-red-600 dark:text-red-400">
              {formatCurrency(phonePeTransactions.filter(t => t.type !== 'credit').reduce((sum, t) => sum + t.amount, 0))}
            </div>
          </div>
          <div className={`p-3 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
